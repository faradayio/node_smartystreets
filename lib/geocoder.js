...
var geocodeChunk = function(item, queue, progress, pool, options, callback){
  ...
  request.post({
    ...
    json: addressList,
    //forever: true, //commented out until https://github.com/request/request/issues/1637 is resolved
    pool: pool, //don't use the default connection pool (for performance)
    timeout: 1000 * 30 //30 second timeout
  }, function(err, response, body){
    if (err || response.statusCode !== 200) {
      var acceptableErrorcodes = ['ETIMEDOUT', 'ECONNRESET', 'ENOTFOUND', 'ESOCKETTIMEDOUT', 'EADDRINFO'];
